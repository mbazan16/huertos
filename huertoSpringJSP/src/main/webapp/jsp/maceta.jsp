...
<body>
	...
	<header>
		<nav>
			<ul>
				...
				<li><a href="${context}/huerto"><fmt:message key="navegacion.huertos"/></a></li>
				<li><a href="${context}/maceta" class="active"><fmt:message key="navegacion.macetas"/></a></li>
	...
	<section id="smodificar" style="display: none;">
		...
		<form id="fmodificar" method="post" action="${context}/maceta/${maceta.id}">
			<input hidden="true" type="text" name="huerto.id" value="${maceta.huerto.id}" />
	...
	<section id="seliminar" style="display: none;">
		<form id="feliminar" method="post" action="${context}/maceta/r/${maceta.id}">
		</form>
	</section>
	...
	<section id="slistado">
		...
		<caption><fmt:message key="listado.plantas"/></caption>
			...
			<c:forEach items="${plantas}" var="planta">
				<tr>
					<td>${planta.nombre}</td>
					<td><a href="${context}/planta/r/${planta.id}"><img src="https://img.icons8.com/color/48/000000/waste--v1.png" width=15 height=15/></a></td>
				</tr>
			</c:forEach>
		</table>
	</section>
	...
	<section id="snuevo" class="modal">
		...
		<form id="fnuevo" method="post" action="${context}/planta">
			<input hidden="true" type="text" name="forward" value="/maceta/${maceta.id}" />
			<input hidden="true" type="text" name="maceta.id" value="${maceta.id}" />
